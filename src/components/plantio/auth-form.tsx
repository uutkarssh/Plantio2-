...
export function AuthForm() {
  ...
  function getRecaptcha() {
    ...
    // Firebase Phone Auth requires app verification with reCAPTCHA on the web.
    // Invisible mode keeps the normal Plantio UI clean; Firebase may still show
    // a challenge when it decides additional verification is necessary.
  ...
  async function sendPhoneCode() {
    ...
    if (!/^\+[1-9]\d{7,14}$/.test(normalized)) {
      return setError("Enter your phone number with country code, for example +919170271488.");
    }

    setLoading(true);
    try {
      const verifier = getRecaptcha();
      confirmationRef.current = await signInWithPhoneNumber(firebaseAuth, normalized, verifier);
      setOtpSent(true);
  ...
  async function verifyPhoneCode() {
    ...
    try {
      ...
      if (mode === "signup" && name.trim().length >= 2 && !credential.user.displayName) {
        await updateProfile(credential.user, { displayName: name.trim() });
      }
          ...
          {forgot ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-2 mb-5">
                ...
                <button type="button" className={secondaryButton} disabled={loading} onClick={() => switchMethod("phone")}>
              ...
              <div className="flex items-center gap-3 mb-5 text-xs font-bold uppercase text-ink/45">
                <div className="h-px flex-1 bg-ink/20" /><span>or {method === "phone" ? "phone verification" : "email"}</span><div className="h-px flex-1 bg-ink/20" />
              </div>
              ...
              {method === "phone" ? (
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block mb-1.5 text-xs font-bold uppercase">Phone number</label>
                    <div className="relative"><Phone className="absolute left-3 top-3.5 w-5 h-5 opacity-50" /><input className={`${inputClass} pl-11`} type="tel" inputMode="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+919170271488" disabled={loading || otpSent} /></div>
                    <p className="mt-1.5 text-[11px] text-ink/50">Firebase may ask for a quick reCAPTCHA check before sending the SMS.</p>
                  </div>
                  {!otpSent ? (
                    <button id="plantio-phone-send" type="button" className={buttonClass} disabled={loading} onClick={sendPhoneCode}>{loading ? <><Loader2 className="inline w-5 h-5 mr-2 animate-spin" /> Sending code...</> : "Send verification code"}</button>
                  ) : (
                    <>
                      <div><label className="block mb-1.5 text-xs font-bold uppercase">6-digit code</label><input className={`${inputClass} text-center tracking-[0.4em] font-bold`} inputMode="numeric" maxLength={6} value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="000000" disabled={loading} /></div>
                      <button type="button" className={buttonClass} disabled={loading} onClick={verifyPhoneCode}>{loading ? <Loader2 className="mx-auto animate-spin" /> : "Verify & continue"}</button>
                      <button type="button" className="w-full text-sm font-bold text-forest" disabled={loading} onClick={() => { setOtpSent(false); confirmationRef.current = null; setOtp(""); clearMessages(); }}>Use a different number</button>
                    </>
                  )}
                  <button type="button" className="w-full flex items-center justify-center gap-2 text-sm font-bold text-forest" onClick={() => switchMethod("email")}><ArrowLeft className="w-4 h-4" /> Use email instead</button>
                  <div id="plantio-phone-recaptcha" aria-hidden="true" />
                </div>
              ) : (
                <div className="space-y-4">
