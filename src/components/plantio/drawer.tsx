...
import { onDrawerOpen } from "@/lib/plantio/drawer-state";
...
export function HamburgerDrawer() {
  ...
  return (
    <>
      {/* The standalone hamburger button is no longer rendered here
          because the BottomNav "More" button now opens the drawer.
          However, we keep a tiny floating trigger for pages where
          the bottom nav might be hidden (edge case). */}
      {!open && (
        <button
          onClick={() => setOpen(true)}
          aria-label="Open menu"
          className="fixed top-4 right-4 z-40 w-12 h-12 rounded-2xl bg-cream border-[3px] border-ink flex items-center justify-center shadow-[3px_3px_0px_0px_#161611] active:translate-x-0.5 active:translate-y-0.5 active:shadow-[1px_1px_0px_0px_#161611] transition-all"
        >
          <TreePine className="w-6 h-6 text-forest" strokeWidth={2.5} />
        </button>
      )}
