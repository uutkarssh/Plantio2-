...
/* Build a numbered Leaflet DivIcon — classic teardrop map-pin shape.
 * Circle head with leaf-green fill + white number, tapering to a pointed needle.
 * iconAnchor is the exact bottom tip of the needle so it sits on the GPS coord.
 */
function pinIcon(number: number): L.DivIcon {
  const headDiameter = 22; // px — the circle at the top
  const needleLen = 10;    // px — the tapered point below the circle
  const totalH = headDiameter + needleLen; // 32px total height
  const halfHead = headDiameter / 2;       // 11px

  return L.divIcon({
    className: "plantio-pin",
    html: `<svg xmlns="http://www.w3.org/2000/svg" width="${headDiameter}" height="${totalH}" viewBox="0 0 ${headDiameter} ${totalH}" style="display:block;">
      <!-- Classic teardrop: needle tip → left of circle → arc over top → right of circle → close to tip -->
      <path d="M${halfHead},${totalH} L0,${halfHead} A${halfHead},${halfHead} 0 1,1 ${headDiameter},${halfHead} Z"
            fill="#8FD14F" stroke="#161611" stroke-width="1.5" stroke-linejoin="round"/>
      <!-- Number label centered in the circle head -->
      <text x="${halfHead}" y="${halfHead}" text-anchor="middle" dominant-baseline="central"
            fill="#FFFFFF" font-size="11" font-weight="700"
            font-family="var(--font-bakbak),'Bakbak One',sans-serif">${number}</text>
    </svg>`,
    iconSize: [headDiameter, totalH],
    iconAnchor: [halfHead, totalH], // bottom tip of the needle = exact GPS coordinate
  });
}
    ...
    >
      ...
      >
        <TileLayer
          ...
          maxZoom={19}
        />
        {/* Transparent reference overlay — place names, admin labels */}
        <TileLayer
          url={ESRI_REF_URL}
          maxZoom={19}
          zIndex={1}
        />
        {/* Transparent road/transportation overlay — roads, highways, rail lines */}
        <TileLayer
          url={ESRI_ROADS_URL}
          maxZoom={19}
          zIndex={2}
      ...
      <div className="absolute bottom-[84px] left-4 right-4 z-[1200]">
        {showResult ? (
          ...
          <StickerCard className="bg-cream p-3">
            ...
            <p className="text-[10px] text-ink/60 mt-1 px-1 leading-snug break-words">
              Satellite tiles need internet — Measure Land won&apos;t work fully offline.
            </p>
          </StickerCard>
        )}
      </div>
