...
/* Session-scoped location cache — avoids re-prompting on every page visit */
const SESSION_LOCATION_KEY = "plantio-session-location";
...
function setCachedLocation(loc: LocationInfo) {
  try {
...
export default function CalendarPage() {
  ...
  /* ---------- fetchWeather helper (stable ref) ---------- */
  ...
  /* ---------- 1. geolocation on mount — real location, no hardcoded fallback ---------- */
  useEffect(() => {
    ...
    try {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          // Permission denied or timeout — show manual fallback, NOT a hardcoded city
          setLocationDenied(true);
          setWeatherLoading(false);
        },
  ...
  /* ---------- Manual city/district geocoding via Nominatim ---------- */
  const geocodeManualLocation = useCallback(async () => {
    if (!manualQuery.trim()) return;
    setManualGeocoding(true);
    setWeatherLoading(true);
    try {
      const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(manualQuery.trim())}&format=json&limit=1`;
      const res = await fetch(url, {
        headers: { "Accept-Language": "en", "User-Agent": "Plantio/1.0" },
    ...
    <main className="flex-1 pb-[calc(env(safe-area-inset-bottom)+96px)] plantio-grain">
      ...
      <section className="px-5 py-6 plantio-section-gap">
        <div className="mx-auto max-w-2xl">
          <div className="flex items-center justify-between gap-2 mb-3">
            ...
            {location?.name && (
              <span className="inline-flex items-center gap-1 text-xs text-ink/70">
                <MapPin className="w-3.5 h-3.5" strokeWidth={2.5} /> {location.name}
              </span>
            )}
          </div>

          {/* Location denied / unavailable — clear message + retry + manual fallback */}
          {locationDenied && !weather && !weatherLoading && (
            <StickerCard className="bg-cream">
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <span className="shrink-0 w-10 h-10 rounded-2xl bg-leaf border-[2.5px] border-ink flex items-center justify-center">
                    <LocateFixed className="w-5 h-5 text-ink" strokeWidth={2.5} />
                  </span>
                  <div>
                    <p className="font-display text-sm font-bold uppercase">{t("calendar.enableLocation")}</p>
                    <p className="text-xs text-ink/70">{t("calendar.enableLocationDesc")}</p>
                  </div>
                </div>
                {/* Retry button */}
                <button
                  onClick={retryGeolocation}
                  className="w-full font-display text-sm font-bold uppercase py-3 rounded-2xl bg-leaf border-[3px] border-ink shadow-[3px_3px_0px_0px_#161611] active:translate-x-0.5 active:translate-y-0.5 active:shadow-[1px_1px_0px_0px_#161611] transition-all flex items-center justify-center gap-2"
                >
                  <RefreshCw className="w-4 h-4" strokeWidth={2.5} /> {t("calendar.retryLocation")}
                </button>
                {/* Manual city/district input */}
                <div className="flex items-center gap-2">
                  <div className="flex-1 relative">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-ink/40" strokeWidth={2.5} />
                    <input
                      type="text"
                      value={manualQuery}
                      onChange={(e) => setManualQuery(e.target.value)}
                      onKeyDown={(e) => e.key === "Enter" && geocodeManualLocation()}
                      placeholder={t("calendar.enterCity")}
                      className="w-full pl-9 pr-3 py-2.5 rounded-xl border-[3px] border-ink font-body text-sm bg-white focus:outline-none focus:ring-2 focus:ring-leaf"
                    />
                  </div>
                  <button
                    onClick={geocodeManualLocation}
                    disabled={!manualQuery.trim() || manualGeocoding}
                    className="shrink-0 w-11 h-11 rounded-xl bg-forest border-[3px] border-ink flex items-center justify-center text-white disabled:opacity-50 active:translate-x-0.5 active:translate-y-0.5 transition-all"
                  >
                    {manualGeocoding ? (
          ...
          {weatherError && !weatherLoading && !locationDenied && (
            <StickerCard className="bg-cream">
              <div className="flex items-center gap-3">
