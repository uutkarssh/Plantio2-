...
export default function HomePage() {
  ...
  // Fetch weather using real geolocation (no hardcoded fallback city)
  useEffect(() => {
    let cancelled = false;

    const fetchWeatherForCoords = async (lat: number, lng: number, locName?: string) => {
      try {
        const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,weather_code`;
        ...
        setWeather({
          ...
          code: c.weather_code,
          loc: locName || `${lat.toFixed(1)}°N, ${lng.toFixed(1)}°E`,
        });
    ...
    try {
      ...
      if (cached) {
        ...
        fetchWeatherForCoords(loc.lat, loc.lng, loc.name);
        return;
      }
    } catch {}
    ...
    if (typeof navigator !== "undefined" && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          if (cancelled) return;
          const loc = { lat: pos.coords.latitude, lng: pos.coords.longitude, isDefault: false };
          try { sessionStorage.setItem("plantio-session-location", JSON.stringify(loc)); } catch {}
          fetchWeatherForCoords(loc.lat, loc.lng);
        },
        () => {
          // Denied — mark as failed (don't show wrong city)
          if (!cancelled) {
            setWeatherFailed(true);
            setWeatherLoading(false);
          }
  ...
  return (
    <main className="flex-1 pb-[calc(env(safe-area-inset-bottom)+96px)] plantio-grain">
      ...
      <section className="relative bg-forest text-white border-b-[3px] border-ink overflow-hidden plantio-torn-edge">
      ...
      {/* DEV ONLY — Database connection health indicator */}
      <DevDbIndicator />
...
/* ─── Dev-only Database Health Indicator ─── */
function DevDbIndicator() {
  ...
  return (
    <section className="px-5 py-3">
      <div className="mx-auto max-w-2xl">
        <div className="inline-flex items-center gap-2 rounded-xl border-[2.5px] border-ink bg-cream px-3 py-1.5 shadow-[2px_2px_0px_0px_#161611]">
          {status === "checking" && (
            <>
              <Loader2 className="w-4 h-4 text-forest animate-spin" strokeWidth={2.5} />
              <span className="font-display text-xs font-bold uppercase text-ink/70">Database: Checking…</span>
            </>
          )}
          {status === "connected" && (
            <>
              <CheckCircle2 className="w-4 h-4 text-leaf" strokeWidth={2.5} />
              <span className="font-display text-xs font-bold uppercase text-forest">Database: Connected ✓</span>
            </>
          )}
          {status === "disconnected" && (
            <>
              <AlertTriangle className="w-4 h-4 text-warn" strokeWidth={2.5} />
              <span className="font-display text-xs font-bold uppercase text-warn">Database: Not Connected ✗</span>
            </>
          )}
          <span className="text-[9px] text-ink/40 font-display uppercase">dev only</span>
        </div>
      </div>
    </section>
  );
