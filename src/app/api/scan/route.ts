...
const UNCERTAIN: ScanResult = {
  plant_name: null, plant_name_hi: null, plant_name_local: null, is_healthy: false,
  disease_name: null, disease_name_hi: null, confidence: 0,
  symptoms_summary: "The plant could not be identified confidently from this image.",
  symptoms_summary_hi: null, plant_description_en: null, plant_description_hi: null,
};
...
async function getFirebaseUid(req: NextRequest): Promise<string | null> {
  ...
  const response = await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=${FIREBASE_API_KEY}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ idToken: token }), cache: "no-store" });
...
async function saveScanToSupabase(req: NextRequest, result: ScanResult, imageDataUrl: string) {
  try {
    ...
    if (!uid) return;
    const { error } = await supabaseServer.from("scan_history").insert({ userId: uid, plant_name: result.plant_name, plant_name_hi: result.plant_name_hi, plant_name_local: result.plant_name_local, is_healthy: result.is_healthy, disease_name: result.disease_name, disease_name_hi: result.disease_name_hi, confidence: result.confidence, symptoms_summary: result.symptoms_summary, symptoms_summary_hi: result.symptoms_summary_hi, plant_description_en: result.plant_description_en, plant_description_hi: result.plant_description_hi, image_url: imageDataUrl });
    if (error) console.error("[Scan] Supabase save failed:", error);
  } catch (error) { console.error("[Scan] Supabase persistence error:", error); }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!image || !image.startsWith("data:image/")) return NextResponse.json({ ok: false, error: "INVALID_IMAGE", message: "A base64 image is required." }, { status: 400 });

    let text: string;
    try {
      text = await runVision(SYSTEM_PROMPT, "Analyze this plant photo and return the JSON diagnosis. Identify the plant if it is reasonably visible, even if the disease cannot be determined confidently.", image);
    } catch (e) {
      console.error("[Scan] AI backend error:", e);
      const message = e instanceof Error ? e.message : "Unknown AI error";
      let error = "AI_ERROR"; let status = 502;
      if (message.includes("429") || message.toLowerCase().includes("rate") || message.toLowerCase().includes("quota")) { error = "AI_RATE_LIMITED"; status = 429; }
      else if (message.includes("503") || message.toLowerCase().includes("unavailable")) { error = "AI_UNAVAILABLE"; status = 503; }
      else if (message.toLowerCase().includes("timed out") || message.toLowerCase().includes("timeout")) { error = "AI_TIMEOUT"; status = 504; }
      else if (message.includes("403") || message.toLowerCase().includes("api key")) { error = "AI_AUTH_ERROR"; status = 502; }
      return NextResponse.json({ ok: false, error, message: "The AI service could not process the scan. Please try again shortly." }, { status });
    }

    const jsonStr = extractJson(text);
    if (!jsonStr) { console.error("[Scan] Gemini returned non-JSON response:", text.slice(0, 1000)); return NextResponse.json({ ok: false, error: "INVALID_AI_RESPONSE", message: "The AI returned an invalid response. Please try again." }, { status: 502 }); }
    let parsed: Partial<ScanResult>;
    try { parsed = JSON.parse(jsonStr); }
    catch (e) { console.error("[Scan] JSON parse error:", e); return NextResponse.json({ ok: false, error: "INVALID_AI_RESPONSE", message: "The AI returned an invalid diagnosis response. Please try again." }, { status: 502 }); }
    ...
    const result: ScanResult = {
      ...
      confidence,
      symptoms_summary: identifiedWithoutDisease ? (typeof parsed.symptoms_summary === "string" && parsed.symptoms_summary.trim() ? parsed.symptoms_summary : "The plant was identified, but no specific disease could be confirmed from this photo.") : (typeof parsed.symptoms_summary === "string" && parsed.symptoms_summary.trim() ? parsed.symptoms_summary : UNCERTAIN.symptoms_summary),
